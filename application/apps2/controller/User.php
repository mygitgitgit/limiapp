...
class User extends Common
{
    ...
    public function login(){
        ...
        $res=Db::name('user')
            ->where('mobile',$mobile)->field('id,code,code_time,identity_time,status,user_info_status,identity_status,pay_password,pay_password_status')
            ->find();
        if($res['status']=='1'){
            apiError('此账号已被禁用！');
            return;
...
//            'money'=>$money
    ...
    public function paymentCodeAction(){
        ...
        //检测验证码是否正确
        $res=Db::name('user')->where('id',$id)->value('code');
        if($res!=$code){
            return apiError('验证码错误');
        }
        // 判断密码是否存在 存在表示重置 不存在表示 设置
        $pass=Db::name('user')->where('id',$id)->value('pay_password');
