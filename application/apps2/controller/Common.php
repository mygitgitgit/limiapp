...
/*****************正则验证函数开始********************/
    ...
    public function checkNickName($param){
        if(!preg_match('/^[\x{4e00}-\x{9fa5}A-Za-z\\s]{2,12}$/u', $param)||preg_match('/\s{2,}/', $param)){
            return true;
        } else {
            return false;
        }
...
/******************写入日志文件函数******************************/
    ...
    public function walletRecordDetail($uid,$rid,$stype,$sid,$status,$money,$wallet,$type,$des){
        Db::name('wallet_record')->insert(['uid'=>$uid,'rid'=>$rid,'stype'=>$stype,'sid'=>$sid,'status'=>$status,'money'=>$money,'wallet'=>$wallet,'type'=>$type,'des'=>$des,'time'=>date('Y-m-d H:i:s')]);
        if(in_array($status,[3,4,5])&&$stype==2){ //退款类不再记录
            return;
        }
        $this->walletRecordToatl($uid,$type,$money,$stype,$status);
    }
...
//                    }
    ...
    public function userInfo($id){
        $res=Db::name('user')
            ...
            ->field(['u.id user_id,u.user_info_status,true_name,u.nickname,sex,signature,head_pic,back_pic,col.name college,sch.name school,gra.name grade'])
            ->where('u.id',$id)
            ->find();
        if($res['sex']=='0'){
            $res['sex']='女';
        ...
        if($res){
            foreach($res as & $v){
                ...
                $count=Db::name('discuss')
                    ...
                    ->where(['action_id'=>$v['action_id'],'parent_id'=>0,'delete_time'=>null])
                    ->select();
                if(!$count){
...
return $res;
    ...
    public function oneInfo($action_id){
        ...
        if($res){
            ...
            $count=Db::name('discuss')
                ->field('count(*) discuss_num')
                ->where(['action_id'=>$res['action_id'],'parent_id'=>0,'delete_time'=>null])
                ->select();
            if(!$count){
                return apiError('响应评论数量有误');
            }
...
//计算距离
    ...
    public function myNum($id){
        ...
        if($data['fans_num']>=10000){
            $data['fans_num']=round($data['fans_num']/10000,1).'万';
        }
        //我的关注数量
        $data['attention_num']=Db::name('user_relation')->where(['user_id'=>$id,'is_cancel'=>0])->count();
        if($data['attention_num']>=10000){
            $data['attention_num']=round($data['attention_num']/10000,1).'万';
        }
        //我拉黑数量
        $data['black_num']=Db::name('user_black')->where(['user_id'=>$id,'is_cancel'=>0])->count();
        if($data['black_num']>=10000){
            $data['black_num']=round($data['black_num']/10000,1).'万';
        }
        //$data['order_num']=$this->order_num($id);
        $data['fans_num']=(string)$data['fans_num'];
        $data['attention_num']=(string)$data['attention_num'];
        $data['black_num']=(string)$data['black_num'];
        return $data;
    }
    /**
     * 我的订单数量
