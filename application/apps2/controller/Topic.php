...
class Topic extends Common
{
    ...
    public function discussList(){
        ...
        $res=Db::name('topic_discuss')
            ...
            ->where(['topic_action_id'=>$topic_action_id,'delete_time'=>null,'parent_id'=>0,'create_time'=>['<=',$time]])
    ...
    public function multiFunction(){
        ...
        if($type=='report'){
            ...
            $res=\app\apps2\model\TopicAction::get($topic_action_id)->delete(); //软删除
            if(!$res){
        ...
        if($res){
            foreach($res as$k=> & $v){
                ...
                $count=Db::name('topic_discuss')
                    ...
                    ->where(['topic_action_id'=>$v['topic_action_id'],'parent_id'=>0,'delete_time'=>null])
                    ->select();
                if(!$count){
                    return apiError('响应评论数量有误');
                }
                $date[$k]['discuss_num']=$v['discuss_num']=$count[0]['discuss_num'];
                //响应的浏览数量
                if($v['view_num']>1000){
    ...
    public function oneAction($id){
        ...
        if($res){
            ...
            $count=Db::name('topic_discuss')
                ->field('count(*) discuss_num')
                ->where(['topic_action_id'=>$res['topic_action_id'],'parent_id'=>0,'delete_time'=>null])
                ->select();
            if(!$count){
                return apiError('响应评论数量有误');
            }
