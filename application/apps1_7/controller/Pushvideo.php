...
//    }
    ...
    public function getNomalVideoList($page,$time){
        ...
        if(!empty($res)){
            foreach ($res as $key=>$val)  {
                ...
                $data[$key]['title']=$this->handleVideoTitle($val['notify_extra'],$val['title']);
                $data[$key]['notify_extra']=$this->handleVideoNoticeExtra($val['notify_extra']);
                $data[$key]['challenge_id']=$val['challenge_id'];
                $data[$key]['challenge']=$val['challenge_name'];
                $data[$key]['publish_addr']=$val['publish_addr'];
                $data[$key]['video_type']=$val['type'];
     ...
     * Remark: 未登录
     */
    public function getStudyNomalVideoList($page,$time){
        $field=' a.id,a.user_id,a.title,a.notify_extra,a.type,a.video_cover,a.video,a.width,a.height,a.view_num,a.click_num,a.discuss_num,a.tags,a.music_id,a.music_type,a.challenge_id,a.challenge_name ,a.publish_addr,b.id user_id,b.nickname user_nickname,CONCAT("'.ApiUrl.'",b.head_pic) AS user_head_pic,c.singer,c.name music_name,CONCAT("'.AliUrl.'",c.pic) AS music_pic,f.name college_name,f.coid college_id';
        $where='a.view_auth=1 AND a.status=0 AND a.type=1';
        $query="SELECT $field FROM limi_video AS a LEFT JOIN limi_user AS b ON a.user_id = b.id LEFT JOIN limi_music AS c ON (a.music_id = c.id AND a.music_type=0)  LEFT JOIN limi_college AS f ON b.college_id=f.coid WHERE $where ORDER BY RAND() LIMIT 20";
        $res=Db::query($query);
        $data=[];
        if(!empty($res)){
     ...
     * Remark:
     */
    public function getStudyRecommVideoList($page,$time,$uid){
        $url='http://video.youhongtech.com';
        $field=' a.id,a.user_id,a.title,a.notify_extra,a.type,a.video_cover,a.video,a.width,a.height,a.view_num,a.music_type,a.challenge_id,a.challenge_name,a.publish_addr, CASE d.type WHEN 1 THEN 1 WHEN 0 THEN 0 ELSE 0 END as is_click,CASE e.is_cancel WHEN 1 THEN 0 WHEN 0 THEN 1 ELSE 0 END as is_attention,a.click_num,a.discuss_num,a.tags,a.music_id,b.id user_id,b.nickname user_nickname,CONCAT("'.ApiUrl.'",b.head_pic) AS user_head_pic,c.singer,c.name music_name,CONCAT("'.AliUrl.'",c.pic) AS music_pic,f.name college_name,f.coid college_id';
        $child_sql="SELECT black_user_id FROM limi_user_black WHERE user_id=$uid";
        $where='a.view_auth=1 AND a.status=0 AND a.type=1 AND a.user_id NOT IN ('.$child_sql.')';
        $query="SELECT $field FROM limi_video AS a LEFT JOIN limi_user AS b ON a.user_id = b.id LEFT JOIN limi_music AS c ON (a.music_id = c.id AND a.music_type=0) LEFT JOIN limi_video_click AS d ON (a.id = d.video_id AND d.user_id=$uid ) LEFT JOIN limi_user_relation AS e ON (e.attention_id=a.user_id AND e.user_id=".$uid." ) LEFT JOIN limi_college AS f ON b.college_id=f.coid WHERE $where ORDER BY RAND() LIMIT 20";
        $res=Db::query($query);
        $data=[];
        if(!empty($res)){
