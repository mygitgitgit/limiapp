...
class Im extends Common
{
    ...
    public function getToken()
    {
        ...
        #查看用户私信设置，返还给客户端
        $private_message_status=Db::name('user')->where('id',$to_uid)->value('send_status');
        if($private_message_status==NULL){
            $im_user['private_message_status']=0;
        }
        $res=Db::name('user_relation')->where('user_id',$uid)->where('attention_id',$to_uid)->where('is_cancel',0)->find();
        if($private_message_status==0){ //禁止未关注者发私信,判断发送方是否是接收方的粉丝
           if(empty($res)){
               $im_user['private_message_status']=0;
           }else{
               $im_user['private_message_status']=1;
           }
        }
        if($private_message_status==1){ //允许未关注着发私信，不管是否关注都可以发
            $im_user['private_message_status']=1;
        }
   ...
   public function getImUinfo(){
    ...
    /**
     * Created by zyjun
     * Info:只用于点赞评论,只发消息accid
     * 调用网易自定义系统消息接口
     */
    public function sendMessage($from,$to,$type,$attach){
        $attach['type']=$type; //0:点赞 1评论   2@好友功能   3关注
        $attach=json_encode($attach);
        $payload['content']="";
        $pushcontent='';
        $payload=json_encode($payload);
