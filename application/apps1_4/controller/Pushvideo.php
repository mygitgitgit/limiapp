...
//    }
     ...
     */
    public function getNomalVideoList($page,$time){
        $url1='http://cloud.youhongtech.com';
        $url2='http://video.youhongtech.com';
        $field=' a.id,a.user_id,a.title,a.video_cover,a.video,a.width,a.height,a.view_num,a.click_num,a.discuss_num,a.tags,a.music_id,a.music_type,b.nickname user_nickname,CONCAT("'.$url1.'",b.head_pic) AS user_head_pic,c.name music_name,CONCAT("'.$url2.'",c.pic) AS music_pic,f.name college_name';
        $where='a.view_auth=1 AND a.status=0';
        $query="SELECT $field FROM limi_video AS a LEFT JOIN limi_user AS b ON a.user_id = b.id LEFT JOIN limi_music AS c ON a.music_id = c.id  LEFT JOIN limi_college AS f ON b.college_id=f.coid WHERE $where ORDER BY RAND() LIMIT 20";
        $res=Db::query($query);
        if(!empty($res)){
            foreach ($res as $key=>$val){
                $res[$key]['title']=$this->userTextDecode($val['title']);
            }
        }
        return $res;
    }
    ...
    public function getRecommVideoList($page,$time,$uid){
        $url1='http://cloud.youhongtech.com';
        $url2='http://video.youhongtech.com';
        $field=' a.id,a.user_id,a.title,a.video_cover,a.video,a.width,a.height,a.view_num, CASE d.type WHEN 1 THEN 1 WHEN 0 THEN 0 ELSE 0 END as is_click,CASE e.is_cancel WHEN 1 THEN 0 WHEN 0 THEN 1 ELSE 0 END as is_attention,a.click_num,a.discuss_num,a.tags,a.music_id,a.music_type,b.nickname user_nickname,CONCAT("'.$url1.'",b.head_pic) AS user_head_pic,c.name music_name,CONCAT("'.$url2.'",c.pic) AS music_pic,f.name college_name';
        ...
        $query="SELECT $field FROM limi_video AS a LEFT JOIN limi_user AS b ON a.user_id = b.id LEFT JOIN limi_music AS c ON a.music_id = c.id LEFT JOIN limi_video_click AS d ON (a.id = d.video_id AND d.user_id=".$uid.") LEFT JOIN limi_user_relation AS e ON (e.attention_id=a.user_id AND e.user_id=".$uid." ) LEFT JOIN limi_college AS f ON b.college_id=f.coid WHERE $where ORDER BY RAND() LIMIT 20";
        $res=Db::query($query);
        if(!empty($res)){
            foreach ($res as $key=>$val){
                $res[$key]['title']=$this->userTextDecode($val['title']);
